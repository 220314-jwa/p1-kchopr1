...
public class StatusController implements CrudHandler {

	
	private StatusServicesImplimentation service;

	public StatusController(StatusServicesImplimentation service) {
		this.service = service;
		if(this.service==null)
			this.service=new StatusServicesImplimentation(new StatusDaoImpli());
	}

	@Override
	public void create(@NotNull Context ctx) {
		Status student = ctx.bodyAsClass(Status.class);
        System.out.println(student);
        System.out.println("create");
        Status temp=service.creat(student);
        if(temp==null)
	...
	public void getOne(@NotNull Context ctx,@NotNull String arg1) {
		...
		Status temp=service.get(Integer.parseInt(arg1));
		if (temp==null)
